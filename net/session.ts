...
export function joinRoom(
  ...
  emotes.clear();
  // 퍼즐 해결 상태도 판마다 새로 시작한다. useGameStore.reset과 달리 이건 지우는 곳이
  // 여기밖에 없다 — 안 지우면 지난 판에 푼 자물쇠가 남아 감방·별관·배수관이 처음부터
  // 열린 채로 시작한다(새로고침하면 사라져 "새로고침 없이 재시작" 때만 보이던 버그).
  useInteraction.getState().resetForNewRoom();
      ...
      onSnapshot: (snap) => {
        ...
        if (snap.solvedIds) {
          for (const id of snap.solvedIds) {
            ...
            // 말풍선은 그 퀴즈 아이템(작업도구함·냉장고 잠금장치…) 머리 위에 뜬다(StampCallouts).
            if (STAMP_QUIZ_ROOM[id]) {
              useStampNotice.getState().show(id);
            }
