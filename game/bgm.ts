...
//
// 2026-07-31 곡 교체: 둘 다 「철문 아래 그림자」의 서로 다른 테이크다(main=1, ending=2).
// 예전엔 서로 다른 곡(Neon Escape / 감옥 탈출)이라 결말에서 정서가 갈아엎어졌다 —
// 같은 곡의 두 판본이면 한 판 전체가 이어진 것처럼 들린다.
//   ⚠️ 곡을 바꾸면 아래 VOLUME(0.25 / 0.45)도 같이 봐야 한다. 음원마다 기본 크기가 달라
//      그대로 두면 메인이 파묻히거나 엔딩이 찢어진다.
//
